...
class Model(nn.Module):
    def __init__(self, cfg, n_layers=2, d_model=512):
        ...
        self.trr = nn.Conv1d(in_channels=cfg.DATASET.SEQLEN, out_channels=cfg.DATASET.SEQLEN, kernel_size=3, stride=1,padding=1, bias=False)
        self.adj16 = nn.Parameter(torch.ones(cfg.DATASET.SEQLEN, cfg.DATASET.SEQLEN))
        self.gcn16 = ModulatedGraphConv(2048,2048,adj=self.adj16)
        self.pe = PositionalEncoding(d_model=2048,max_len=cfg.DATASET.SEQLEN)
        self.ffn16 = PoswiseFeedForwardNet(in_channels=cfg.DATASET.SEQLEN)
