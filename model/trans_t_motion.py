...
class Model(nn.Module):
    def __init__(self, cfg, n_layers=2, d_model=512):
        ...
        self.center = (cfg.DATASET.SEQLEN - 1) // 2
    ...
    def forward(self, x, is_train=False, J_regressor=None):
        ## motion
        x_motion = x.clone()  # B 16 2048
        x_motion[:, 1:-1, :] = x[:, 1:-1, :] - 0.5 * x[:, 2:, :] - 0.5 * x[:, :-2, :]
        x_motion[:, 0, :] = 0
        x_motion[:, -1, :] = 0

        x = torch.cat([x.unsqueeze(1), x_motion.unsqueeze(1)], dim = 1) # B 2 16 2048
        x = self.fusion(x).squeeze(1) # B 16 2048
...
# {'mpjpe': 83.2, 'mpjpe_pa': 51.2, 'accel_err': 8.45, 'mpvpe': 98.54}
